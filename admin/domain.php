...
function getTCY($url) {
    ...
    $uri = "http://bar-navig.yandex.ru/u?ver=2&url=".urlencode("http://".$url)."&show=1";
...
function getUrl($url) {
    ...
    //если XML файл прочитан, то возвращаем значение параметра value, иначе возвращаем false - ошибка
    return $xml ? (int) substr(strstr($xml, 'url="'), 3) : false;
}

$tcy = getTCY($_POST['search']);

$url = getUrl($_POST['search']);

echo '<br><br><strong>Тиц:</strong> ' . $tcy;

echo '<br><br><strong>Урл:</strong> ' . $url;
